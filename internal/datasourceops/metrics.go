...
func (s *Service) GetDatasourceMetrics(ctx context.Context, id string) (DatasourceMetrics, error) {
	ds, err := s.requireDatasource(id)
	if err != nil {
		return DatasourceMetrics{}, err
	}
	return s.collectDatasourceMetrics(ctxOrBackground(ctx), ds, ""), nil
}

func (s *Service) GetDatasourceMetricsByNode(ctx context.Context, id, node string) (DatasourceMetrics, error) {
	ds, err := s.requireDatasource(id)
	if err != nil {
		return DatasourceMetrics{}, err
	}
	return s.collectDatasourceMetrics(ctxOrBackground(ctx), ds, node), nil
}

func (s *Service) collectDatasourceMetrics(ctx context.Context, ds datasource.DataSource, redisNode string) DatasourceMetrics {
	...
	case datasource.TypeRedis:
		s.collectRedisMetrics(ctx, ds, redisNode, &metrics, appendWarning)
	case datasource.TypeElasticsearch:
		s.collectElasticsearchMetrics(ctx, ds, &metrics, appendWarning)
	case datasource.TypeMongoDB:
		s.collectMongoMetrics(ctx, ds, &metrics, appendWarning)
	case datasource.TypeMySQL:
		s.collectMySQLMetrics(ctx, ds, &metrics, appendWarning)
	case datasource.TypePostgreSQL:
		s.collectPostgresMetrics(ctx, ds, &metrics, appendWarning)
	default:
...
func (s *Service) collectRedisMetrics(ctx context.Context, ds datasource.DataSource, requestedNode string, metrics *DatasourceMetrics, warn func(string)) {
	requestedNode = normalizeRedisNodeAddress(requestedNode)
	explicitNodeRequested := requestedNode != ""

	clusterNodes := s.loadRedisClusterNodes(ctx, ds)
	...
	if len(clusterNodes) > 0 {
		...
		if selectedNode != "" {
			...
			if err == nil {
				s.collectRedisMetricsCore(ctx, metricsDS, selectedNode, metrics, warn)
	...
	if explicitNodeRequested {
		...
		s.collectRedisMetricsCore(ctx, metricsDS, requestedNode, metrics, warn)
		return
	}
	s.collectRedisMetricsCore(ctx, ds, "", metrics, warn)
}

func (s *Service) collectRedisMetricsCore(ctx context.Context, ds datasource.DataSource, selectedNode string, metrics *DatasourceMetrics, warn func(string)) {
	memResult, err := s.requiredManager().ExecuteInternal(ctx, ds, "INFO memory", console.ExecuteOptions{})
	if err != nil {
	...
	cpuResult, err := s.requiredManager().ExecuteInternal(ctx, ds, "INFO cpu", console.ExecuteOptions{})
	if err != nil {
...
func (s *Service) collectElasticsearchMetrics(ctx context.Context, ds datasource.DataSource, metrics *DatasourceMetrics, warn func(string)) {
	...
	result, err := s.requiredManager().ExecuteInternal(ctx, ds, statement, console.ExecuteOptions{})
	if err != nil {
	...
	var cpuSum, cpuCount float64
	var memoryUsed, memoryMax int64
...
func (s *Service) collectMongoMetrics(ctx context.Context, ds datasource.DataSource, metrics *DatasourceMetrics, warn func(string)) {
	result, err := s.requiredManager().ExecuteInternal(ctx, ds, "db.serverStatus()", console.ExecuteOptions{})
	if err != nil {
...
func (s *Service) collectMySQLMetrics(ctx context.Context, ds datasource.DataSource, metrics *DatasourceMetrics, warn func(string)) {
	result, err := s.requiredManager().ExecuteInternal(ctx, ds, "SELECT COALESCE(SUM(CURRENT_NUMBER_OF_BYTES_USED),0) AS memory_used_bytes FROM performance_schema.memory_summary_global_by_event_name", console.ExecuteOptions{})
	if err == nil {
	...
	totalResult, err := s.requiredManager().ExecuteInternal(ctx, ds, "SELECT @@innodb_buffer_pool_size AS memory_total_bytes", console.ExecuteOptions{})
	if err == nil {
...
func (s *Service) collectPostgresMetrics(ctx context.Context, ds datasource.DataSource, metrics *DatasourceMetrics, warn func(string)) {
	usedResult, err := s.requiredManager().ExecuteInternal(ctx, ds, "SELECT COALESCE(SUM(total_bytes),0) AS memory_used_bytes FROM pg_backend_memory_contexts", console.ExecuteOptions{})
	if err == nil {
	...
	totalResult, err := s.requiredManager().ExecuteInternal(ctx, ds, "SELECT pg_size_bytes(current_setting('shared_buffers')) AS memory_total_bytes", console.ExecuteOptions{})
	if err == nil {
		if row, ok := firstRow(totalResult); ok {
	...
	cpuResult, err := s.requiredManager().ExecuteInternal(ctx, ds, "SELECT COALESCE(SUM(user_time + system_time),0) AS cpu_seconds FROM pg_stat_kcache", console.ExecuteOptions{})
	if err == nil {
...
func (s *Service) loadRedisClusterNodes(ctx context.Context, ds datasource.DataSource) []string {
	result, err := s.requiredManager().ExecuteInternal(ctx, ds, "CLUSTER NODES", console.ExecuteOptions{})
	if err != nil {
		return nil
	}
...
func parseRedisClusterNodes(raw string) []string {
	...
	for _, line := range lines {
		...
		if addr := normalizeRedisNodeAddress(fields[1]); addr != "" {
...
func redisNodesFromDatasourceOptions(ds datasource.DataSource) []string {
	...
	case []any:
		for _, node := range typed {
			if addr := normalizeRedisNodeAddress(fmt.Sprint(node)); addr != "" {
				nodes = append(nodes, addr)
			}
		}
	case []string:
		for _, node := range typed {
			if addr := normalizeRedisNodeAddress(node); addr != "" {
				nodes = append(nodes, addr)
			}
		}
	case string:
		...
		for _, part := range parts {
			if addr := normalizeRedisNodeAddress(part); addr != "" {
				nodes = append(nodes, addr)
			}
		}
	}
...
func normalizeRedisNodeAddress(value string) string {
	...
	if strings.Contains(node, "@") {
		node = node[strings.LastIndex(node, "@")+1:]
	}
...
func formatMetricBytes(value int64) string {
	...
	return fmt.Sprintf("%.2f EB", f/unit)
}
